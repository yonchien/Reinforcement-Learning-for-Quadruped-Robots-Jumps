"""This file mirriors locomotion_gym_env.py from google motion-imitation,
to be used as an RL env.

Still a lot of cleaning up to do. Some TODO:
-add reliable NOISE to reset()
-add action filter to aliengo, limit amount any joint can move within some amount of time
...
import attr
import typing
# sys.path.append('/Users/guillaume/Documents/Research/USC_DRL/motion_imitation/motion_imitation/')
# sys.path.append('/Users/guillaume/Documents/Research/USC_DRL/motion_imitation/motion_imitation/robots/')
# import laikago

from usc_learning.envs.google_robots import laikago
...
class SimulationParameters(object):
  ...
  num_action_repeat = attr.ib(type=int, default=10)
...
class LaikagoGymEnv(gym.Env):
  ...
  def __init__(self,
    ...
    print('action config before len ', len(action_config),"\n",action_config)
    if self._envNoHips:
      action_indices = [1,2, 4,5, 7,8, 10,11]
      #del action_config[0::3]
      action_config = [action_config[i] for i in action_indices]
    print('action config after len ', len(action_config),"\n",action_config)
    ...
    obs_dim = len(self._robot.GetTrueObservation())
    obs_low = np.array([-100] * obs_dim )
    obs_high = np.array([100] * obs_dim )
  ...
  def reset(self,
    ...
    motor_angles = motor_angles + 0.3 * 2 * (0.5 - np.random.random(len(motor_angles)))
    motor_angles[0::3] = 0 # set hips back to 0
    if np.random.random() > 0.3:
      self._robot._SettleDownForReset(motor_angles,self._robot.time_step)
  ...
  def step(self, action):
    ...
    """
    if self._envNoHips:
      action = np.array([0, action[0], action[1], 0, action[2], action[3], 
                         0, action[4], action[5], 0, action[6], action[7]])
  ...
  def _termination(self):
    """ Aggressive - end early if dips too far down or basevel is too large"""
    ...
    # for s in self.all_sensors():
    #   s.on_terminate(self)
    if self._env_time_step * self._env_step_counter > 5:
      return True

    fallThreshold = 1
    basepos = self._robot.GetBasePosition()
    basevel = np.array(self._robot.GetBaseVelocity())
    baserpy = self._robot.GetTrueBaseRollPitchYaw()
    if abs(baserpy[0]) > fallThreshold or abs(baserpy[1]) > fallThreshold:
      return True
    # if basepos[2] < 0.25 or any(basevel > 1):
    #   return True
    
    return False

  def _reward(self, desHeight=0.48):
    basepos = self._robot.GetBasePosition()
    baserpy = self._robot.GetTrueBaseRollPitchYaw()[0:2]

    x = abs(basepos[2] - desHeight)
    sigma = 0.07
    reward = np.exp(-0.5 * (x/sigma)**2) #- 0.1*np.linalg.norm(baserpy)
    #reward = desHeight - abs(basepos[2] - desHeight) - 1e-4*np.linalg.norm(baserpy)
    return reward
  ...
  def _get_observation(self):
    ...
    observations = self._robot.GetTrueObservation()
    return observations
