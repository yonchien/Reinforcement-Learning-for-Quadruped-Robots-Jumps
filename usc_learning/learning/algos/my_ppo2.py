...
class Runner(AbstractEnvRunner):
    """Editing runner to only use observations and actions actually used by environment. 

    For example, if we step the environment with balance controller actions, 
    the agent has no way of knowing this, and the updates from using whichever action it sampled 
    will not mean much.
    ...
    def _run(self):
        ...
        while real_num_steps < self.n_steps:
            ...
            #print(infos)
            if infos[0]['num_non_rl_actions'] > 0:
                continue
            #print('real steps:', real_num_steps)
            real_num_steps += 1
