...
class TransformerNetTF(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
                 name):
        super(TransformerNetTF, self).__init__(
        ...
        action_seq_len = custom_model_config.get("action_seq_len", 1)
        ...
        inputs_sensor_data = inputs[..., :-12]
        inputs_prev_action = inputs[..., -12:]

        inputs_sensor_data = tf.keras.layers.Reshape(target_shape=(obs_seq_len, 60))(inputs_sensor_data)
        inputs_prev_action = tf.keras.layers.Reshape(target_shape=(action_seq_len, 12))(inputs_prev_action)

        transformer_out, attention_weights = Transformer(num_layers=2, d_model=64, num_heads=8, d_ff=128,
                                                         output_dim=num_outputs, pe_input=pe_max_len,
                                                         pe_target=pe_max_len, dropout_rate=0)(
            [inputs_sensor_data, inputs_prev_action], training=is_training)
        ...
        action_out = tf.keras.layers.Dense(
            ...
            name="fc_out",
            activation=None,
            kernel_initializer=normc_initializer(0.01))(transformer_out)
